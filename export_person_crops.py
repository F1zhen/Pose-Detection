...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export person crops for manual sit/stand labeling."
    )
    parser.add_argument("--input", type=Path, default=Path("data"))
    parser.add_argument("--output-dir", type=Path, default=Path("datasets/pose_classifier/unlabeled"))
    parser.add_argument("--model", default=DEFAULT_MODEL)
    parser.add_argument("--device", default="auto")
    parser.add_argument("--conf", type=float, default=0.15)
    parser.add_argument("--iou", type=float, default=0.55)
    parser.add_argument("--imgsz", type=int, default=1280)
    parser.add_argument("--frame-step", type=int, default=15, help="Export every Nth frame.")
            ...
            if result.boxes is not None and len(result.boxes) > 0:
                ...
                for det_index, bbox in enumerate(boxes):
                    ...
                    crop_name = (
                        ...
                        f"id{track_id if track_id is not None else 'na'}_"
